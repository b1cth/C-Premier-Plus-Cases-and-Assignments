...
public:
	...
	SingingWaiter(const Waiter& wt, int p=0, int v = other) :Worker(wt), Singer(wt, v), Waiter(wt,p) {}
	SingingWaiter(const Singer& wt, int p = 0, int v = other) :Worker(wt), Singer(wt,v), Waiter(wt, p) {}
	virtual void set();
	virtual void show() const;
};